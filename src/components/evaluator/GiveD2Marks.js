...
export default function GiveD2MarksEvaluator() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        const groupsRes = await api.get('/api/evaluator/groups/own', { params: { defenseType: 'd2' } });
    ...
    return (
      <div className="max-w-4xl mx-auto mt-6">
        <div className="mb-4">
          ...
          <p className="text-sm text-gray-500">Loading your assigned D2 groups…</p>
        </div>
  ...
  if (error) {
    return (
      <div className="max-w-4xl mx-auto mt-6">
        <div className="mb-4">
          <h1 className="text-2xl font-semibold text-gray-900 mb-1">Give D2 marks</h1>
          <p className="text-sm text-gray-500">Enter or view D2 evaluation marks for your groups.</p>
        </div>
    ...
    <div className="max-w-4xl mx-auto mt-6">
      ...
      {groups.length === 0 ? (
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-8 text-center text-gray-500">
          No D2 groups found for you in the active session.
        </div>
      ) : (
