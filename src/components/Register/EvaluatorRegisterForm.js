...
export default function EvaluatorRegisterForm({ onBack, onSubmit, onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await api.post('/api/evaluators/register', payload);
    ...
    <>
      ...
      >
        ...
        >
         ...
         <a href="/evaluator-login" className=' className="bg-transparent border-0 p-0 text-accent font-medium cursor-pointer hover:underline focus:outline-none focus:ring-2 focus:ring-accent/30 rounded"'>Log in</a>
