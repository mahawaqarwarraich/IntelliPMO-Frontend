...
export default function PanelAssignmentD2() {
  ...
  return (
    <div className="max-w-5xl mx-auto mt-6">
      <div className="mb-4">
        <h1 className="text-2xl font-semibold text-gray-900 mb-1">Panel assignment for D2</h1>
        <p className="text-sm text-gray-500">
          Assign FYP groups to D2 panels. Select a panel, then choose groups to associate with it.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-6 sm:p-7 space-y-6">
          <div className={fieldWrapClass}>
            ...
            {panels.length === 0 && (
              <p className="text-xs text-amber-600">No D2 panels found for the active session. Create panels first.</p>
            )}
          </div>
              ...
              <div className="max-h-64 overflow-y-auto rounded-lg border border-gray-200 divide-y divide-gray-100">
                {availableGroups.map((g) => {
                  ...
                  return (
                    <label
                      key={g._id}
                      className={`flex items-start gap-3 p-3 cursor-pointer hover:bg-gray-50/80 ${checked ? 'bg-accent/5' : ''}`}
                    >
                      <input
                        type="checkbox"
                        checked={checked}
          ...
          <div className="pt-2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <p className="text-xs text-gray-500">After creating an assignment you can select another panel and assign more groups.</p>
            <button
              type="button"
