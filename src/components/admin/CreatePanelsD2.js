...
export default function CreatePanelsD2() {
  ...
  useEffect(() => {
    ...
    api
      .get('/api/evaluators', { params: { defenseType: 'd2' } })
      .then((res) => {
  ...
  return (
    <div className="max-w-5xl mx-auto mt-6">
      <div className="mb-4">
        <h1 className="text-2xl font-semibold text-gray-900 mb-1">Create panels for D2</h1>
        <p className="text-sm text-gray-500">Create evaluator panels using D2 evaluators from the active session.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-6 sm:p-7 space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div className={`${fieldWrapClass} lg:col-span-1`}>
              <label htmlFor="searchEvaluator" className={labelClass}>
                Search evaluator
              </label>
              <input
                id="searchEvaluator"
          ...
          <div className="pt-2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <p className="text-xs text-gray-500">Tip: Use search to quickly find evaluators by name, then add them to the panel.</p>
            <button
              type="button"
