...
export default function ManageStudents() {
  ...
  const handleDelete = (student) => {
    ...
    if (!window.confirm(`Are you sure you want to delete ${student.studentName} (${student.rollNo})?`)) {
      return;
    }
      ...
      <div className="max-w-4xl mx-auto">
        <header className="mb-8">
          ...
          <p className="text-gray-600 mt-1 text-sm sm:text-base">
            {activeSessionYear
              ? `List of students for the session ${activeSessionYear}.`
              : 'List of students for the active session.'}
          </p>
        </header>
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <section className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[400px]" aria-label="Students list">
            <thead>
            ...
            <tbody>
              {students.length === 0 ? (
                ...
                students.map((student) => (
                  <tr key={student._id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50/50">
                    <td className="py-3 px-4 text-sm text-gray-600">{student.number}</td>
