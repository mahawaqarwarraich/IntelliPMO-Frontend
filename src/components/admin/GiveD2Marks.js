...
export default function GiveD2Marks() {
    ...
    return (
      <div className="max-w-4xl mx-auto mt-6">
        <div className="rounded-xl border border-amber-200 bg-amber-50 shadow-sm p-6 sm:p-8">
          <h1 className="text-xl font-semibold text-gray-900 mb-2">Give D2 marks</h1>
  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto mt-6">
        <div className="mb-4">
          <h1 className="text-2xl font-semibold text-gray-900 mb-1">Give D2 marks</h1>
          <p className="text-sm text-gray-500">Enter or view D2 evaluation marks.</p>
        </div>
  ...
  return (
    <div className="max-w-4xl mx-auto mt-6">
      <div className="mb-4">
        <h1 className="text-2xl font-semibold text-gray-900 mb-1">Give D2 marks</h1>
        <p className="text-sm text-gray-500">Registered groups (overall status approved). Select a group to enter or view D2 marks.</p>
      </div>

      {groups.length > 0 && (
        <div className="mb-4">
          <label htmlFor="idea-name-filter-d2" className="block text-sm font-medium text-gray-700 mb-1">
            Filter by idea name
          </label>
          <input
            id="idea-name-filter-d2"
            type="text"
        ...
        <div className="space-y-4">
          {filteredGroups.map((g) => (
            <div
              ...
              role="button"
              tabIndex={0}
              onClick={() => navigate(`/dashboard/give-d2-marks/group/${g._id}`)}
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault();
                  navigate(`/dashboard/give-d2-marks/group/${g._id}`);
                }
              }}
