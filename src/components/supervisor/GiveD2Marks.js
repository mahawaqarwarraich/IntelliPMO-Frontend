...
export default function GiveD2MarksSupervisor() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        const groupsRes = await api.get('/api/supervisor/groups/own');
    ...
    <div className="max-w-4xl mx-auto mt-6">
      ...
      {groups.length === 0 ? (
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-8 text-center text-gray-500">
          No groups found for you in the active session.
        </div>
      ) : (
