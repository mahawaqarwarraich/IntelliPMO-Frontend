...
export default function SupervisorGroupRequest() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((sessionRes) => {
        ...
        return api.get('/api/supervisor/groups');
      })
      .then((res) => {
        if (res?.data?.groups != null) {
          ...
          const messages = {};
          list.forEach((g) => {
            approvals[g._id] = g.supervisorStatus === 'accepted' ? 'accepted' : 'rejected';
            messages[g._id] = g.supervisorMessage ?? '';
          });
  ...
  const handleSave = (groupId) => {
    const approval = localApprovals[groupId] ?? 'rejected';
    const message = localMessages[groupId] ?? '';
    setSavingId(groupId);
    api
      .patch(`/api/supervisor/groups/${groupId}`, { approval, message })
      .then(() => {
              ...
              <table className="w-full min-w-[600px] text-left">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50/80">
                    ...
                    <th className="py-3 px-4 text-[13px] font-semibold text-gray-700">Admin approval</th>
                ...
                <tbody>
                  <tr>
                    ...
                    <td className="py-3 px-4 text-sm text-gray-900 align-top">
                      {g.adminStatus === 'accepted' ? 'Accepted' : g.adminStatus === 'rejected' ? 'Rejected' : 'Pending'}
                    </td>
