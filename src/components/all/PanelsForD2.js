...
export default function PanelsForD2() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        const panelsRes = await api.get('/api/panels', { params: { defenseType: 'd2' } });
    ...
    return (
      <div className="max-w-4xl mx-auto mt-6">
        <div className="rounded-xl border border-amber-200 bg-amber-50 shadow-sm p-6 sm:p-8">
          <h1 className="text-xl font-semibold text-gray-900 mb-2">Panels for D2</h1>
  ...
  return (
    <div className="max-w-4xl mx-auto mt-6">
      <div className="mb-4">
        <h1 className="text-2xl font-semibold text-gray-900 mb-1">Panels for D2</h1>
        <p className="text-sm text-gray-500">D2 panels, their members and assigned groups for the active session.</p>
      </div>
      ...
      {error ? (
        ...
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-8 text-center text-gray-500">
          No D2 panels found for the active session.
        </div>
      ) : (
                ...
                {panels.map((p) => (
                  <tr key={p._id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700">
                      {Array.isArray(p.memberNames) && p.memberNames.length > 0 ? p.memberNames.join(', ') : '—'}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-700">
